...
async function sendMail(options: { to: string; subject: string; html: string }) {
  ...
  await transporter.sendMail({
    from: process.env.SMTP_FROM || "Florista Plants <no-reply@florista.tn>",
    ...options,
  });
}
  ...
  await sendMail({
    ...
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:auto;padding:32px;background:#f6fff8;border-radius:16px">
        ...
        <p style="color:#aaa;font-size:12px">Florista — Khniss, Monastir 5011, Tunisie</p>
      </div>
    `,
  });
}
...
export async function sendPasswordResetEmail(email: string, name: string, token: string) {
  ...
  await sendMail({
    to: email,
    subject: "Réinitialisation de mot de passe — Florista",
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:auto;padding:32px;background:#f6fff8;border-radius:16px">
        <h1 style="color:#2F6F4E;font-size:28px;margin-bottom:8px">🌿 Florista</h1>
...
export async function sendOrderConfirmationEmail(
  ...
  await sendMail({
    to: email,
    subject: `Confirmation commande #${orderId} — Florista`,
    html: `
      <div style="font-family:sans-serif;max-width:600px;margin:auto;padding:32px;background:#f6fff8;border-radius:16px">
        <h1 style="color:#2F6F4E;font-size:28px;margin-bottom:8px">🌿 Florista</h1>
        ...
        <p style="color:#aaa;font-size:12px;margin-top:24px">Florista — Khniss, Monastir 5011, Tunisie</p>
      </div>
    `,
  });
}
